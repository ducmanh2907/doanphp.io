...
require_once(APP_PATH . '/admin/models/AdminUserModel.php');
...
class AdminCartController {
    ...
    public function __construct($db) {
        ...
        $this->userModel = new AdminUserModel($db); // Giả sử bạn có UserModel để lấy thông tin người dùng
    }
    ...
    foreach ($cartItems as $item) {
       ...
       require_once APP_PATH . '/admin/views/cart/view.php';
    ...
    public function add($userId) {
        if (isset($_POST['product_id']) && isset($_POST['quantity'])) {
            ...
            if ($this->cartModel->addToCart($userId, $productId, $quantity)) {
                ...
                header("Location: /webbanhang/app/admin/index.php?controller=admincart&action=index&user_id={$userId}");
                exit();
            } else {
